...
class MainCommand(BaseSQLModel, table=True):
    ...
    def validate_params_format(self):
        ...
        if params is None and format is None:
            return self
        
        if params is None or format is None:
            raise ValueError(
                "Params and format must both be None or have the same number of comma seperated values"
            )
        
        paramslist = params.split(",")
        formatlist = format.split(",")

        if len(paramslist) != len(formatlist):
            raise ValueError(
